...
const outputDir = process.env.OUTPUT_DIR || "page5-menu-event-listener-probe";
...
const breakpointEvents = ["listener:touchend", "listener:click"];
const maxPauses = 12;
...
async function listProperties(cdp, objectId, limit = 120) {
  if (!objectId) return [];
  ...
  return (response.result || []).slice(0, limit).map((descriptor) => {
    ...
    const geometry = ["x", "y", "width", "height", "clientx", "clienty", "pagex", "pagey", "screenx", "screeny", "button", "buttons", "visible", "alpha"].includes(lower);
    const value = summarizeRemote(descriptor.value);
    if (value && !geometry) delete value.value;
    return {
...
async function evaluateFrame(cdp, callFrameId, expression) {
  try {
    const response = await cdp.send("Debugger.evaluateOnCallFrame", {
      callFrameId,
      expression,
      objectGroup: "gams-event-listener-probe",
      silent: true,
      returnByValue: false,
      generatePreview: false,
    });
    return {
      ...
      properties: response.result?.objectId ? await listProperties(cdp, response.result.objectId, 100) : [],
    };
  } catch (error) {
...
const report = {
  ...
  mode: "page5-menu-event-listener-read-only-probe",
...
try {
        ...
        for (const frame of (event.callFrames || []).slice(0, 8)) {
          ...
          for (const scope of (frame.scopeChain || []).slice(0, 8)) {
            ...
            if (["local", "closure", "catch", "block"].includes(scope.type) && scope.object?.objectId) {
              try { properties = await listProperties(cdp, scope.object.objectId, 100); } catch (error) { debuggerErrors.push(String(error).slice(0, 500)); }
            }
            scopeSummary.push({
              type: scope.type,
              name: scope.name || "",
  ...
  for (const eventName of breakpointEvents) {
    await cdp.send("DOMDebugger.setEventListenerBreakpoint", { eventName });
  }
  ...
  for (const eventName of breakpointEvents) {
    try { await cdp.send("DOMDebugger.removeEventListenerBreakpoint", { eventName }); } catch {}
  }
  ...
  report.pass = report.firstPauseCaptured === true
    && pauses.length > 0
  ...
  try { await cdp.send("Runtime.releaseObjectGroup", { objectGroup: "gams-event-listener-probe" }); } catch {}
  await context.close();
} catch (error) {
...
console.log(JSON.stringify({
  ...
  pauseEvents: (report.pauses || []).map((item) => item.data?.eventName || item.reason),
