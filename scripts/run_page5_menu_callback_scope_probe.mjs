...
function summarizeRemote(remote) {
  ...
  const result = {
    ...
    description: String(remote.description || "").slice(0, 260),
  ...
  if (typeof remote.value === "object" && remote.value !== null) result.rawValue = remote.value;
  return result;
}

function shouldDescend(name, depth, rootMode) {
  if (depth <= 0) return false;
  if (rootMode) return true;
  const lower = String(name || "").toLowerCase();
  return lower.includes("scene")
...
async function describeRemote(cdp, remote, depth = 1, visited = new Set(), rootMode = false) {
  ...
  const properties = [];
  for (const descriptor of (response.result || []).slice(0, 240)) {
    const entry = {
      name: descriptor.name,
      value: summarizeRemote(descriptor.value),
      hasGetter: Boolean(descriptor.get),
      hasSetter: Boolean(descriptor.set),
    };
    if (descriptor.value?.objectId && shouldDescend(descriptor.name, depth, rootMode)) {
      try {
        entry.child = await describeRemote(cdp, descriptor.value, depth - 1, visited, false);
      } catch (error) {
        entry.childError = String(error).slice(0, 500);
      }
    }
  ...
  summary.internalProperties = [];
  for (const internal of (response.internalProperties || []).slice(0, 60)) {
    const entry = { name: internal.name, value: summarizeRemote(internal.value) };
    if (internal.value?.objectId && depth > 0) {
      try {
        entry.child = await describeRemote(cdp, internal.value, depth - 1, visited, true);
      } catch (error) {
        entry.childError = String(error).slice(0, 500);
      }
    }
    summary.internalProperties.push(entry);
  }
  return summary;
}
...
async function evaluateFrame(cdp, callFrameId, expression) {
  const response = await cdp.send("Debugger.evaluateOnCallFrame", {
    callFrameId,
    expression,
    objectGroup: "gams-menu-scope-probe",
    silent: true,
    returnByValue: false,
    generatePreview: false,
  });
...
try {
  ...
  cdp.on("Debugger.paused", async (event) => {
    ...
    try {
      ...
      const evaluated = await evaluateFrame(cdp, frame.callFrameId, expression);
      if (!evaluated.result?.objectId) throw new Error("menu callback function unavailable");
      const sourceResult = await cdp.send("Runtime.callFunctionOn", {
        objectId: evaluated.result.objectId,
        functionDeclaration: "function(){return Function.prototype.toString.call(this);}",
        returnByValue: true,
        silent: true,
      });
      const described = await describeRemote(cdp, evaluated.result, 4, new Set(), true);
  ...
  report.pauseCaptured = await Promise.race([
    pauseReady,
    new Promise((resolve) => setTimeout(() => resolve(false), 18000)),
  ]);
  ...
  try { await cdp.send("Runtime.releaseObjectGroup", { objectGroup: "gams-menu-scope-probe" }); } catch {}
  await context.close();
} catch (error) {
...
console.log(JSON.stringify({
  ...
  callbackSourceLength: report.callback?.sourceLength || 0,
  callbackPropertyCount: report.callback?.structure?.propertyCount || 0,
  callbackInternalProperties: (report.callback?.structure?.internalProperties || []).map((item) => item.name),
