...
if len(sys.argv) != 6:
    raise SystemExit(
        "usage: build_remote_engine_ab_runtime.py <noname.js> <game.js> "
        "<version> <runtime-master-key> <output-dir>"
    )
...
remote_route = b"https://gams-script-edge.2320006072.workers.dev/engine/stable.js"
virtual_route = b"https://ggv2.local/runtime/game.js"
if remote_route not in noname:
    raise SystemExit("remote engine route is missing from A/B source")
if virtual_route in noname:
    raise SystemExit("virtual engine route must not be present in A/B source")
if noname.decode("utf-8").encode("utf-8") != noname:
    raise SystemExit("noname source is not an exact UTF-8 round trip")

output.mkdir(parents=True, exist_ok=True)
plain_zip = output / ".runtime-ab.zip"
with zipfile.ZipFile(plain_zip, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
    archive.writestr("noname.js", noname)
...
Path("/tmp/remote-engine-ab-runtime-canonical.txt").write_text(
    canonical, encoding="utf-8", newline="\n"
)
Path("/tmp/remote-engine-ab-runtime-unsigned.json").write_text(
