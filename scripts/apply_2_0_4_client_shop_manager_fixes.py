...
def patch_remote_script() -> None:
    path = Path("remote-script/src/noname.js")
    text = path.read_text(encoding="utf-8")
    text = replace_once(text, "// @version      1.1.3", "// @version      1.1.4", "remote version")
    text = replace_once(
        text,
        'subtitle: "状态：2026年7月23日正常使用"',
        'subtitle: "状态：2026年8月4日正常使用"',
        "remote status date",
    )
...
'''
    new = '''              if(vUYe8N&&yWpiJH&&Epe456s) {
                const XBbHBMQ=window[Epe456s],
                ggOriginalCallback=XBbHBMQ&&XBbHBMQ["__ggOriginalCallback"]||XBbHBMQ,
                ggBuyNumber=parseInt(yWpiJH,
                10),
                ggResponse= {
                  ["status"]:1,
                  ["msg"]:"succes"+"sful",
                  ["data"]: {
                    ["goods_"+"id"]:vUYe8N,
                    ["order_"+"id"]:zCSo6J["generateTimestamp"](),
                    ["buy_nu"+"m"]:ggBuyNumber
                  }
                };
                window["商品购买数量"]=ggBuyNumber;
                if(typeof ggOriginalCallback==="functi"+"on") {
                  Promise["resolve"]()["then"](()=>ggOriginalCallback["call"](window,
                  ggResponse))
                }
...
'''
    text = replace_once(text, old, new, "repeat shop purchase JSONP hook")
...
'''
    ...
    text = replace_once(text, "source:'native-2.0.2'", "source:'native-2.0.4'", "native marker")
    path.write_text(text, encoding="utf-8")
...
if __name__ == "__main__":
    ...
    patch_client()
    patch_manager()
    print("GG 2.0.4 client/shop and manager fixes applied")
