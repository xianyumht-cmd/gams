...
def main() -> int:
    ...
    text = replace_once(
        ...
        'const outputDir = process.env.OUTPUT_DIR || "page5-target-side-entry-probe";',
        "output",
    )
    ...
    route_block = '''const routes = [
  {
    ...
    steps: [
      ...
      { label: "target-side-init", x: 258, y: 732 },
      { label: "target-side-open", x: 258, y: 732 },
    ],
    marker: "target-read",
  },
];'''
    ...
    text = replace_once(
        ...
        'mode: "page5-target-side-entry-probe"',
        "mode",
    )
