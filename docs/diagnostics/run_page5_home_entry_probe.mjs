...
const probes = [
  { name: "title-progress", taps: [{ x: 90, y: 400 }, { x: 90, y: 400 }, { x: 90, y: 400 }, { x: 90, y: 400 }], waitAfterEachMs: 1800 },
...
async function pointInfo(page, point) {
  return page.evaluate(({ x, y }) => {
    ...
    return {
      ...
      text: String(element.innerText || element.getAttribute?.("aria-label") || "").replace(/\s+/g, " ").trim().slice(0, 240),
...
async function runProbe(browser, probe, index) {
  ...
  await context.route("**/*", async (route) => {
    ...
    const topNavigation = request.isNavigationRequest() && request.frame().parentFrame() === null;
    requests.push({
      at: Date.now(),
      kind,
    ...
    if (kind === "order-request") {
      blockedOrders.push({ at: Date.now(), method: request.method(), url: safeUrl(rawUrl), postDataLength: request.postDataBuffer()?.length || 0 });
      await route.abort("blockedbyclient");
      return;
    }
    if (topNavigation && !String(rawUrl).startsWith(targetPrefix)) {
      blockedExternalNavigations.push({ at: Date.now(), method: request.method(), url: safeUrl(rawUrl) });
      await route.abort("blockedbyclient");
      return;
    }
  ...
  try {
    ...
    for (let tapIndex = 0; tapIndex < probe.taps.length; tapIndex += 1) {
      ...
      const beforeRequestIndex = requests.length;
      ...
      const stageRequests = requests.slice(beforeRequestIndex).filter((item) => item.at >= tapAt);
      const kinds = {};
      for (const item of stageRequests) kinds[item.kind] = (kinds[item.kind] || 0) + 1;
      result.screenshots[stageName] = await capture(page, path.join(outputDir, `${prefix}-${stageName}.png`));
      result.stages.push({
        stage: stageName,
        tapAt,
        point,
        ...
        screenshotChangedFromPrevious: result.screenshots[stageName] !== result.screenshots[tapIndex === 0 ? "initial" : `after-tap-${tapIndex}`],
        requestWindow: {
...
const summary = {
  ...
  validMainFrameCases: cases.filter((item) => item.mainFrameValidBefore && item.mainFrameValidAfter).length,
  screenshotChangedCases: cases.filter((item) => item.stages?.some((stage) => stage.screenshotChangedFromPrevious)).length,
  targetListCases: cases.filter((item) => item.stages?.some((stage) => stage.requestWindow?.targetListCount > 0)).length,
  targetReadCases: cases.filter((item) => item.stages?.some((stage) => stage.requestWindow?.targetReadCount > 0)).length,
